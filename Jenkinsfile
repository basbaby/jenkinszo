   ...
   stages {
        	...
        	steps {
					...
					container_Up = false
                    			timeout(time: 1, unit: "HOURS") { 
                        			sh "curl -u admin:admin -X GET -H \"Accept: application/json\" http://104.248.169.167:9000/api/qualitygates/project_status?projectKey=com.mycompany:jenkinszo > status.json"
	 ...
	 stage('Build') {
        ...
        stage('Deploy to Cloudhub'){
        	steps {
				 withCredentials([file(credentialsId: 'settings', variable: 'settings')]){
        	    	sh "mvn -f pom.xml -s $settings package deploy -DskipTests -Dusername=joji1 -Dpassword=Canadavisa25@ -DapplicationName=jenkinszo-poc -Denvironment=Sandbox -DmuleDeploy -Dcloudhub.region=us-east-2 -Danypoint.platform.client_id=fda777bd3e3b4fcb93aff995fea2043d -Danypoint.platform.client_secret=4193AA1986054C548Bf757fd1B7F6f18 -Dkey=mymulesoft"
